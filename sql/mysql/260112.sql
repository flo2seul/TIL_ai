...
SELECT * FROM member;
SELECT mem_id, mem_name, mem_bir, mem_job as job, mem_mileage FROM member;
-- 직업이 회사원인 회원 조회
SELECT mem_name, mem_milege, mem_mileage * 0.1 as ex1, mem_mileage * mem_mileage as ex2, mem_mileage + mem_mileage as ex3 FROM member WHERE mem_job = "회사원";
-- 직업이 주부이며 취미가 수영인 회원 조회
SELECT * FROM member WHERE mem_job = "주부" AND mem_like = "수영";
-- 마일리지가 5000 이상 
SELECT mem_name, mem_mileage FROM member WHERE mem_mileage >= 5000;
-- 마일리지가 2700 초과 6000미만 회원 조회
SELECT mem_name, mem_mileage FROM member WHERE mem_mileage > 2700 AND mem_mileage < 6000 ORDER BY mem_mileage;
-- job 내림차순, like 오름차순
SELECT mem_name, mem_job, mem_like FROM member ORDER BY mem_job DESC, mem_like;
-- 직업 : 자영업, 은행원, 농업 회원 조회하시오.
SELECT mem_name, mem_job FROM member WHERE mem_job IN("자영업","은행업","농업") ORDER BY mem_name ASC;
-- 회원 이름이 김으로 시작하는 회원 조회
SELECT * FROM member WHERE mem_name LIKE "김%";
-- 대전 서구에 사는 회원 조회
SELECT * FROM member WHERE mem_add1 LIKE "대전시 서구%";
-- 
